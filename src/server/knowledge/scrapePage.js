...
export async function scrapePage(url, conditional = {}) {
  ...
  try {
    ...
    // Links pra documento (editais, formulários, portarias... em PDF ou
    // Word) — coletados ANTES de remover nav/menu/sidebar, porque é comum
    // esse tipo de documento estar linkado justamente num menu lateral, não
    // no corpo do texto.
    ...
    $(
      ...
      if (!href) return;
      try {
        documentLinks.add(new URL(href, url).href);
      } catch {
